...
from .models import Category, Custumer, CartProduct, Product, Order
...
class ProductDetailView(CartMixin,  DeleteView):
...
class DelFromCartView(CartMixin, View):
...
class MakeOrderView(CartMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        form = OrderForm(request.POST or None)
        customer = Custumer.objects.get(user=request.user)
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.INFO, 'Заказ оформлен. Мы свяжемся с вами в ближайшее время')
